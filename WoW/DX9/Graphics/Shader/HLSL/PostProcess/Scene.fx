                    ...
                    out float4 oDepth : COLOR3 )
{
    ...
    float3 albedo = tex2D(g_samScene, Tex).rgb;
    float3 diffuse = Diffuse.rgb * albedo.rgb;
    float3 ambient = float3(0.7f, 0.7f, 0.7f) * albedo.rgb;
    oCol = float4(diffuse + ambient, 1);
    //
    // Output normal
    //
    ...
    float3 depth = (ClipPos.z) / 100.0f;
    oDepth = float4(depth, 1.0f);
