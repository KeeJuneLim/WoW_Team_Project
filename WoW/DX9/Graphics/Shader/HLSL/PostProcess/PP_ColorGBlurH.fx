...
sampler2D g_samSceneDepth = sampler_state
{
    Texture = <g_txSceneDepth>;
    AddressU = Clamp;
    AddressV = Clamp;
...
float4 PostProcessPS( float2 Tex : TEXCOORD0 ) : COLOR0
{
    float4 Color = 0;
    float depth = 0;
    depth = tex2D(g_samSceneDepth, Tex);
    if (depth > 0.2f && depth < 0.5f)
    {
    ...
    else
    {
        ...
        {
            float2 kernel;
            kernel.x = TexelKernel[i].x / gWidth;
            kernel.y = TexelKernel[i].y / gHeight;
            Color += tex2D(g_samSrcColor, Tex + kernel.xy) * BlurWeights[i];
        }
    }
