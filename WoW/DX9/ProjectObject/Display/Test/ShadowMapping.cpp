...
void ShadowMapping::Init()
{
	...
	if (FAILED(D3DXLoadMeshFromX(xPath, D3DXMESH_MANAGED, DX::GetDevice(), NULL, NULL, NULL, NULL, &m_pTorus)))
		assert(false && "Loading x file is failed");

	xPath = _T("../../_Assets/shader/disc.x");
	if (FAILED(D3DXLoadMeshFromX(xPath, D3DXMESH_MANAGED, DX::GetDevice(), NULL, NULL, NULL, NULL, &m_pDisc)))
		assert(false && "Loading x file is failed");
...
void ShadowMapping::Render()
{
	...
	for (int i = 0; i < numPasses; i++)
	{
		...
		color = D3DXVECTOR4(0, 0, 1, 0);
		worldMatrix = matT_torus;
		m_pApplyShadow->SetVector("gObjectColor", &color);
		m_pApplyShadow->SetMatrix("gWorldMatrix", &worldMatrix);
		m_pApplyShadow->CommitChanges();
		m_pDisc->DrawSubset(0);
